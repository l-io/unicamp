...
def main():
    ...
    for img, img_name in zip(images, images_name):
        ...
        cv2.imwrite(os.path.join(path, 'output',
                    "{}-{}".format('output-mono', img_name)), img_mono)

        # Contour output
        img_contour = np.ones(img_mono.shape, np.uint8)*255
        contours, _ = cv2.findContours(
            img_mono, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(img_contour, contours, -1, 0)
        cv2.imwrite(os.path.join(path, 'output',
                    "{}-{}".format('countour', img_name)), img_contour)
        ...
        cv2.imwrite(os.path.join(path, 'output',
                    "{}-{}".format('regions', img_name)), img_regions)
        ...
        plt.hist(area_list, bins=[0, 1500, 3000, 4500],
                 edgecolor='black', color="#3F5D7D")
        plt.xlim([0, 4500])
        plt.ylabel('Número de Objetos')
        plt.xlabel('Área')

        plt.savefig(os.path.join(
            path, 'output', "{}-{}".format('histogram', img_name)), bbox_inches='tight')
